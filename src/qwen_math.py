...
def compute_score(solution_str: str, ground_truth: Any) -> float:
    ...
    try:
        ...
        # Debug: print first few cases to see what's happening
        # import random
        # if random.random() < 0.02:  # Print ~2% of cases for debugging
        #     print(f"DEBUG qwen_math:")
        #     print(f"  solution: '{solution_str[:150]}...'")
...
def extract_answer(pred_str: str) -> str:
    ...
    tag_answer = re.findall(r"<answer>(.*?)</answer>", pred_str)
    if tag_answer:
        return tag_answer[-1]
...
def symbolic_equal(a, b):
    ...
    if not LATEX2SYMPY_AVAILABLE:
        try:
            parsed_a = parse_expr(a)
            parsed_b = parse_expr(b)
            result = simplify(parsed_a) == simplify(parsed_b)
            return result
        except:
            return False
